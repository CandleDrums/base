...
public abstract class CustomBaseSeerviceImpl<VO, DO> implements BaseService<VO> {
    ...
    public List<VO> queryPagingList(VO params, Integer startIndex, Integer pageSize) {
        if (CheckUtils.isEmpty(params, pageSize))
            return null;
        return getVOList(getDAO().queryPagingList(getDO(params, doType), startIndex, pageSize), voType);
    }

...
public class ObjectUtils {
     ...
     * </p>
     * <p>
     * For example, in a {@code HashMap} the {@link java.util.HashMap#get(java.lang.Object)} method returns {@code null}
     * if the {@code Map} contains {@code null} or if there is no matching key. The {@code Null} placeholder can be used
    ...
    /**
     * <p>
     * {@code ObjectUtils} instances should NOT be constructed in standard programming. Instead, the static methods on
     * the class should be used, such as {@code ObjectUtils.defaultIfNull("a","b");}.
     * </p>
     * <p>
    ...
    /**
     * <p>
     * Returns the first value in the array which is not {@code null}. If all the values are {@code null} or the array
     * is {@code null} or empty then {@code null} is returned.
     * </p>
     *
     * <pre>
     ...
     * @return the first value from {@code values} which is not {@code null}, or {@code null} if there are no non-null
     * values
     * @since 3.0
     */
    @SafeVarargs
    ...
    /**
     * <p>
     * Gets the toString that would be produced by {@code Object} if a class did not override toString itself.
     ...
     * @param c1 the first comparable, may be null
     * @param c2 the second comparable, may be null
     * @param nullGreater if true {@code null} is considered greater than a non-{@code null} value or if false
     * {@code null} is considered less than a Non-{@code null} value
     * @return a negative value if c1 &lt; c2, zero if c1 = c2 and a positive value if c1 &gt; c2
    ...
    /**
     * Find the "best guess" middle value among comparables. If there is an even number of total values, the lower of
     * the two middle values will be returned.
     * 
     ...
     * </p>
     * <p>
     * This method is similar to {@link #clone(Object)}, but will return the provided instance as the return value
     * instead of {@code null} if the instance is not cloneable. This is more convenient if the caller uses different
     * implementations (e.g. of a service) and some of the implementations do not allow concurrent processing or have
     * state. In such cases the implementation can simply provide a proper clone implementation and the caller's code
    ...
    // Constants (LANG-816):
    /*
     * These methods ensure constants are not inlined by javac. For example, typically a developer might declare a
     * constant like so: public final static int MAGIC_NUMBER = 5; Should a different jar file refer to this, and the
     * MAGIC_NUMBER is changed a later date (e.g., MAGIC_NUMBER = 6), the different jar file will need to recompile
     * itself. This is because javac typically inlines the primitive or String constant directly into the bytecode, and
     * removes the reference to the MAGIC_NUMBER field. To help the other jar (so that it does not need to recompile
     * when constants are changed) the original developer can declare their constant using one of the CONST() utility
     * methods, instead: public final static int MAGIC_NUMBER = CONST(5);
     */
     ...
     * </pre>
     *
     * This way any jars that refer to this field do not have to recompile themselves if the field's value changes at
     * some future date.
     *
     ...
     * @throws IllegalArgumentException if the value passed to v is larger than a byte, that is, smaller than -128 or
     * larger than 127.
     * @return the byte v, unchanged
     * @since 3.2
     */
     ...
     * @throws IllegalArgumentException if the value passed to v is larger than a short, that is, smaller than -32768 or
     * larger than 32767.
     * @return the byte v, unchanged
     * @since 3.2
     */
